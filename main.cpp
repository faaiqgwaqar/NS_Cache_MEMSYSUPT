...
bool GCRAM_MUXEN = true; // Enable/Disable Use of Multiplexing on SenseAmps, although this might not be incredibly useful for us
...
double GCRAM_GATECAP = (5.5e-15 - GCRAM_CDREDUCTION * 2) /** 0.388*/; // What is the gate capacitance of the device per micron
double GCRAM_CONTCAP = (1.9e-15 - GCRAM_CDREDUCTION) /** 0.0379*/; // What is the source/drain capacitance of the device per micron, this is parasitic
double GCRAM_CHANRES = 30769.3 /** 1.6*/; // What is the channel resistnace at 1 micron. This has an inverse relationship to width when compared to cap

// Device Geometry Parameters
double GCRAM_RACCW = 150e-3; // What is the width of the read access transistor?
...
double GCRAM_WACCT = 250e-12 /** 1.6 * 0.388*/; // 62.6e-12*/; // What is the write access latency of the cell

// Voltage Operation Parameters
double GCRAM_WBVOLT = 0.7; // What is the voltage used to activate the write bitlines?
double GCRAM_WWVOLT = 1.6; // What is the voltage used to activate the write wordlines?
...
// 3D Decoding Stack Parameters
double GCRAM_DECLAYER = 1;
bool GCRAM_2TGC = false;
bool GCRAM_3TGC = true;

int numRowSubarray, numColumnSubarray;
...
int nvsim(ofstream& outputFile, string inputFileName, long long& numSolution, Result *bestDataResults, Result *bestTagResults)
{
	...
	int numActiveSubarrayPerRow, numActiveSubarrayPerColumn;
	...
	BIGFOR {
		...
		if (!dataBank->invalid) {
			...
			tempResult.printAsCacheToFile(inputParameter->cacheAccessMode, "tempResults/tempResult"+ std::to_string(numSolution)+".nsd");
